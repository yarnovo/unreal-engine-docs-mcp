...
async function fetchNavigation(): Promise<void> {
  ...
  try {
    ...
    console.log("📄 访问文档页面...");
    const url =
      "https://dev.epicgames.com/documentation/zh-cn/unreal-engine/unreal-engine-5-6-documentation";
    ...
    let expandedCount = 0;
    const maxAttempts = 10; // 最多尝试10轮展开
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      const expandButtons = await page.$$(
        ".btn-expander .icon-arrow-forward-ios:not(.is-rotated)"
      );
      ...
      for (let i = 0; i < expandButtons.length; i++) {
        try {
          // 检查按钮是否仍然存在且未展开
          const isStillCollapsed = await expandButtons[i].evaluate(
            (el) => !el.classList.contains("is-rotated")
          );
          ...
          const clicked = await expandButtons[i].evaluate((el) => {
            const expandButton = el.closest(".btn-expander") as HTMLElement;
            if (expandButton) {
              expandButton.click();
              return true;
            }
          ...
          console.log(
            `   ⚠️  展开第 ${i + 1} 项时出错: ${(error as Error).message}`
          );
        }
      }
    ...
    const linkCount = await page.evaluate(() => {
      return document.querySelectorAll(".documents-tree a.contents-table-link")
        .length;
    });
    ...
    console.error("❌ 获取导航失败:", (error as Error).message);
