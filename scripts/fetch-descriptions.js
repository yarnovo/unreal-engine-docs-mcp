...
// 生成随机浏览器标识
function generateRandomBrowserFingerprint() {
  const userAgents = [
...
async function simulateHumanBehavior(page) {
  try {
    ...
    console.log(`   🤖 模拟人类行为时出错: ${error.message}`);
  }
}

// 处理单个页面的函数
async function processSinglePage(page, linkInfo, index, total) {
  ...
  try {
    ...
    console.log(`   [${index}] ❌ 处理失败: ${error.message}`);
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
      ...
      for (const page of pages) {
        try {
          ...
          console.log(`   ⚠️  关闭标签页时出错: ${error.message}`);
        }
      }
