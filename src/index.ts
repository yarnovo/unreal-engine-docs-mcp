...
try {
  ...
  try {
    ...
    console.warn("无法加载页面描述数据，将使用基础链接数据:", 
  ...
  console.log(`📊 其中 ${enhancedDocLinks.filter(link => link.pageTitle).length} 个有页面标题`);
  console.log(`📊 其中 ${enhancedDocLinks.filter(link => link.pageDescription).length} 个有页面描述`);
...
server.tool(
  "get_docs_list",
  {
    search: z.string().optional().describe("可选的搜索关键字，用于模糊匹配导航标题、页面标题和页面描述"),
  },
  async (args) => {
    ...
    const result = {
      ...
      links: filteredLinks.map(link => ({
        ...
        link: link.link,
        // 为了兼容性，保留原有的 title 和 description 字段
        title: link.title,
